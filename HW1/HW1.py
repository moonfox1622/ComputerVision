...
def process(floder):
    light_pos = np.zeros((6,3),dtype=np.float64)
    ...
    for i in range(0,6):
        ...
        picture_list = np.array(picture_gray, dtype=np.uint64)
        H,W = picture_gray.shape
        
        if i==0 :
            #預計存進6張圖片的灰階值
            rgb = np.zeros((6,H,W),dtype=np.uint64) 
